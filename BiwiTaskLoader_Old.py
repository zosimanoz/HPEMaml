...
class BiwiTaskLoader():
    def __init__(self, root, batch_size, n_way, k_shot, k_query):
        ...
        self.x_train = self.x[:10000]
        self.x_test = self.x[3219:]
        ...
        for sample in range(5):  # num of eisodes
            ...
            for i in range(self.batchsz):  # one batch means one set
                ...
                for j, cur_class in enumerate(selected_cls):
                    ...
                    im = data_pack[cur_class][selected_img[:self.k_shot][0]]
                ...
                x_spt = np.array(x_spt).reshape(self.n_way * self.k_shot, 64,64, 3)[perm]
                y_spt = np.array(y_spt).reshape(self.n_way * self.k_shot, 3)[perm]
                perm = np.random.permutation(self.n_way * self.k_query)
                x_qry = np.array(x_qry).reshape(self.n_way * self.k_query, 64,64, 3)[perm]
                ...
                # [b, setsz, 1, 84, 84]
            x_spts = np.array(x_spts).astype(np.float32).reshape(self.batchsz, setsz, 3, 64, 64)
            y_spts = np.array(y_spts).astype(np.int).reshape(self.batchsz, setsz, 3)
            # [b, qrysz, 1, 84, 84]
            x_qrys = np.array(x_qrys).astype(np.float32).reshape(self.batchsz, querysz, 3, 64, 64)
