...
class Learner(nn.Module):
    ...
    def forward(self, support_x, support_y, query_x, query_y, num_updates):
        ...
        support_x = np.moveaxis(support_x, 3, 1)

        query_x = np.array(query_x)
        query_x = np.moveaxis(query_x, 3, 1)

        support_x = torch.from_numpy(support_x).float()
        query_x = torch.from_numpy(query_x).float()

        # plt.imshow(support_x[0][0,:,:])
        # plt.show()
        ...
        return loss, loss_mae, gradient_phi, angles
...
class MetaLearner(nn.Module):
    ...
    def forward(self, support_x, support_y, query_x, query_y):
        ...
        for i in range(meta_batch_size):
            ...
            loss, mae, grad_phi, angles = self.learner(support_x[i], support_y[i], query_x[i], query_y[i], self.num_updates)
    ...
    def pred(self, support_x, support_y, query_x, query_y):
        ...
        mae_losses = []
        for i in range(meta_batch_size):
            loss, loss_mae, grad_phi, angles = self.learner(support_x[i], support_y[i], query_x[i], query_y[i],  self.num_updates)
            ...
            # img1 = support_x[i][0].numpy()
            # img1 = np.moveaxis(img1, 0, 2)
            # plt.imshow(img1)
            # plt.show()


            img = query_x[i][0].numpy()

            img = np.moveaxis(img, 2, 0)

            y = query_y[i][0][0].item()
            p = query_y[i][0][1].item()
            r = query_y[i][0][2].item()

            img = utils.draw_axis(img, y, p, r)
